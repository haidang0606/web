...
function Home() {
  ...
  const [listCategories, setListCategories] = React.useState([]); //Create listCustomers State
  ...
  const RetrieveAllCategories = () => {
    ...
    CRUD.getAllCate().then((res) => {
      console.log(res);
      setListCategories(res.data.data); //Set list customers after get all result from server
